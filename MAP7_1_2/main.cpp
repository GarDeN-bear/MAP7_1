...
int main() {
	...
	for (const auto& count : countCPU) {
		...
		for (const auto& vectorSize : vectorsSizes) {
			...
			auto start = std::chrono::steady_clock::now(); // запоминаем время начала выполнения
			...
			auto stop = std::chrono::steady_clock::now(); // запоминаем время окончания выполнения
			auto duration = std::chrono::duration_cast<std::chrono::microseconds> (stop - start); // вычисляем время выполнения в микросекундах
			std::cout << static_cast<double>(duration.count()) / 1e6 << "s\t";
			//std::cout << "sum: " << sum << std::endl; // проверка верности расчёта
			sum = 0;
		}
		std::cout << std::endl;
	}
